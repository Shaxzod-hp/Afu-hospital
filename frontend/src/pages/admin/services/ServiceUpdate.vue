<template>
  <div class="management-page">
    ...
    >
      <h5 class="m-0 fw-800">
        <i class="fas fa-edit me-2"></i> Xizmatni tahrirlash
      </h5>
      <router-link
        to="/admin/services"
    ...
    <div v-else-if="loadError" class="content-card glass-panel p-5 text-center">
      ...
      <button class="btn btn-sm btn-outline-danger mt-3" @click="fetchService">
        ...
        <div class="row g-4">
          ...
          <div class="col-12">
            ...
            <input
              ...
              placeholder="Masalan: Laboratoriya tahlillari"
              required
            />
          </div>
...
const fetchService = async () => {
  ...
  loadError.value = "";
  try {
    const data = await servicesService.fetchOne(route.params.id);
    const service = data?.data || data;

    form.value = {
      name: service.name || "",
      short_description: service.short_description || "",
      items: Array.isArray(service.included_items)
        ? service.included_items.map((i) => ({ name: i.name, price: i.price }))
        : [],
      photoFile: null,
    };
    photoPreview.value = getPhotoUrl(service.photo);
  } catch (err) {
    loadError.value = "Xizmat ma'lumotlarini yuklashda xatolik yuz berdi.";
  } finally {
    loading.value = false;
  }
};
